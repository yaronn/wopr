...
DocumentViewer.prototype.readOptions = function(node, ctor) {
   ...
   for (var i=0; i<items.length; i++) {
      ...
      var optionsProto = optionsMethod==null ? null : optionsMethod()
      var tokens = item.name.split('-')
      
      res[tokens[0]] = res[tokens[0]] || {}
      if (optionsProto!=null) optionsProto=optionsProto[tokens[0]]
      var last = res
      for (var j=1; j<tokens.length; j++) {
         last[tokens[j-1]][tokens[j]] = last[tokens[j-1]][tokens[j]] || {}
         last = last[tokens[j-1]]
         if (optionsProto!=null) optionsProto=optionsProto[tokens[j]]
      }
      
      var val = this.convert(item.value, optionsProto)
